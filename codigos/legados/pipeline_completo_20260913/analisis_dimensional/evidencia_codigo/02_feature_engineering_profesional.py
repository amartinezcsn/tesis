...
def main() -> None:
    ...
    # Compatibilidad con la salida vigente de 01_clean_eda.py. El limpiador
    # conserva el nombre técnico ``inpc_indice``, mientras que este módulo y
    # la configuración metodológica publican la variable como valor mensual.
    if "inpc_valor_mensual" not in master.columns:
        if "inpc_indice" not in master.columns:
            raise KeyError(
                "El dataset maestro no contiene 'inpc_indice' ni "
                "'inpc_valor_mensual'."
            )
        master["inpc_valor_mensual"] = master["inpc_indice"]
