"""Entrada de compatibilidad del pipeline semanal híbrido Rev44."""

import subprocess
import sys
from pathlib import Path

from config_semanal import DAILY_MASTER_PATH, WEEKLY_MASTER_PATH, WEEKLY_MODEL_PATH, ensure_output_dir, PROJECT_DIR
from trazabilidad import write_experiment_manifest


STAGES = (
    "02_agregar_semanal.py",
    "03_features_semanales.py",
    "04_perfil_semanal.py",
    "04_analisis_series_temporales_semanal.py",
    "06_modelos_rolling_window.py",
    "07_reportes_semanales.py",
    "08_exportar_dss_semanal.py",
)


def legacy_main() -> None:
    """Ejecuta el flujo antiguo de modelos individuales, solo como antecedente."""
    scripts_dir = Path(__file__).resolve().parent
    if not DAILY_MASTER_PATH.exists():
        raise FileNotFoundError(
            "No existe el maestro diario requerido: "
            f"{DAILY_MASTER_PATH}. Ejecuta antes el pipeline maestro de limpieza."
        )
    for stage in STAGES:
        stage_path = scripts_dir / stage
        if not stage_path.exists():
            raise FileNotFoundError(f"No existe la etapa semanal: {stage_path}")
        print(f"\n=== Etapa semanal: {stage} ===")
        subprocess.run([sys.executable, str(stage_path)], check=True, cwd=PROJECT_DIR)
    write_experiment_manifest(
        ensure_output_dir() / "00_trazabilidad_ejecucion.json",
        {"maestro_diario": DAILY_MASTER_PATH, "maestro_semanal": WEEKLY_MASTER_PATH, "dataset_modelado": WEEKLY_MODEL_PATH},
        [scripts_dir / stage for stage in (*STAGES, "config_semanal.py", "trazabilidad.py")],
    )
    print("Manifiesto generado: output/semanal/00_trazabilidad_ejecucion.json")


def main(argv=None) -> int:
    """Ejecuta Rev44 también cuando otro módulo importa y llama a main()."""
    from hibrido.cli import main as hybrid_main
    return hybrid_main(list(argv) if argv is not None else (sys.argv[1:] or ["audit"]))


if __name__ == "__main__":
    raise SystemExit(main())
