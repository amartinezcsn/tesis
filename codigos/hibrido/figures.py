...
def generate_figures(output,purchases,rejected,coverage,panel=None,results=None,controls=None,demo=False):
    ...
    def save(number,figure,table,chapter='Desarrollo'):
        ...
        note=('DEMOSTRACIÓN SINTÉTICA — NO ES EVIDENCIA DE TESIS' if demo else 'Fuente: registros auditados de la ejecución')
    ...
    states={'observada':0,'cero_confirmado':1,'incompleta':2,'desconocida':3}
    ...
    if panel is not None and results is not None:
        ...
        if len(dev)>=10 and dev.total.std()>0:
    ...
    for number,title in enumerate(TITLES):
        if number not in generated:
            ...
            if number==8:reason='STL requiere al menos 156 semanas de desarrollo en esta configuración.'
