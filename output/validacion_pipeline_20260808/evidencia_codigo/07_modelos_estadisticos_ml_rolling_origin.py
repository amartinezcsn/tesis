...
def buscar_mejores_hiperparametros(
    ...
    expected_origins = len(selected_origins)
    summary = (
    ...
    summary["origenes_esperados"] = expected_origins
    summary["cobertura_completa"] = summary["origenes_evaluados"].eq(expected_origins)
    summary["es_mejor"] = False
    best_params: dict[str, dict[str, Any]] = {}
    complete = summary.loc[summary["cobertura_completa"]].copy()
    missing_models = sorted(set(grids) - set(complete["modelo"]))
    if missing_models:
        raise RuntimeError(
            "Ningun candidato cubrio todos los origenes para: " + ", ".join(missing_models)
        )
    for model_name, group in complete.groupby("modelo", sort=False):
        group = group.sort_values(["rmse_promedio", "mae_promedio", "candidato"])
...
def main() -> None:
    ...
    for target in TARGET_COLUMNS:
        target_df, excluded_trailing_days = trim_trailing_inactive_target(
            df, target, FINAL_EVALUATION_ORIGINS * FORECAST_HORIZON_DAYS
        )
        origins = rolling_origins(len(target_df))
        if not origins:
            raise ValueError(
                f"El objetivo '{target}' requiere mas de "
                f"{ROLLING_INITIAL_DAYS + FORECAST_HORIZON_DAYS} filas para Rolling-Origin."
            )
        if ENABLE_HYPERPARAMETER_TUNING:
            available_tuning_origins, evaluation_origins = split_tuning_and_evaluation_origins(
                len(target_df), FINAL_EVALUATION_ORIGINS
            )
            if not available_tuning_origins:
                raise ValueError(
                    f"No hay origenes anteriores a la evaluacion final para ajustar '{target}'."
                )
            ...
            best_by_target[target] = DEFAULT_MODEL_PARAMS
        target_results = rolling_origin_evaluation(
            target_df,
            {target: best_by_target[target]},
            evaluation_origins,
            targets=[target],
        )
        target_results["dias_cola_sin_cobertura_excluidos"] = excluded_trailing_days
        result_frames.append(target_results)
        coverage_rows.append(
            {
                "dataset": DATASET_TO_USE,
                "target": target,
                "fecha_inicio_disponible": target_df[DATE_COLUMN].min().date(),
                "fecha_fin_efectiva": target_df[DATE_COLUMN].max().date(),
                "dias_cola_sin_cobertura_excluidos": excluded_trailing_days,
                "origenes_ajuste": len(available_tuning_origins) if ENABLE_HYPERPARAMETER_TUNING else 0,
    ...
    best_configuration = (
        ranking.loc[ranking.groupby(["dataset", "target"])["rmse_promedio"].idxmin()]
        .sort_values(["dataset", "target"])
        .reset_index(drop=True)
    )
    best_configuration.insert(0, "criterio_seleccion", "menor_rmse_promedio_evaluacion_final")
    best_rows = [
        {
            "dataset": DATASET_TO_USE,
            "target": target,
    ...
    with pd.ExcelWriter(output_xlsx, engine="openpyxl") as writer:
        ...
        best_configuration.to_excel(writer, sheet_name="mejor_configuracion", index=False)
        pd.DataFrame(coverage_rows).to_excel(writer, sheet_name="cobertura_objetivos", index=False)
