...
def sklearn_scores_if_available(train: pd.DataFrame, predictors: list[str], target: str) -> pd.DataFrame:
    ...
    # La regularizacion tambien debe respetar el orden cronologico. KFold, que
    # es el valor implicito de LassoCV cuando cv es un entero, mezcla bloques
    # futuros con pasados dentro del conjunto de desarrollo.
    temporal_cv = TimeSeriesSplit(n_splits=5)
    lasso = make_pipeline(
        StandardScaler(),
        LassoCV(cv=temporal_cv, random_state=42, max_iter=20000),
    )
    lasso.fit(x, y)
...
def select_features(scores: pd.DataFrame) -> pd.DataFrame:
    ...
    # La version anterior comparaba el score compuesto normalizado contra un
    # umbral definido para correlacion absoluta. Esa mezcla de escalas hacia
    # que casi todas las variables quedaran seleccionadas (264 de 271), por lo
    # que el supuesto dataset reducido no reducia realmente dimensionalidad.
    ranking["seleccionada"] = ranking["rank_target"] <= TOP_FEATURES_PER_TARGET
    return ranking
