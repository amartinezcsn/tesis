...
## Figura 2. Ventana supervisada de 28 días

- **Archivo:** `02_ventana_supervisada_28_dias.png`
- **Fuente:** 08_rnn_lstm_dataset_reducido.py
- **Hoja:** make_sequences
- **Variables:** x[i-28:i], y[i]
- **Objetivo:** Ilustrar cómo cada ejemplo de entrenamiento utiliza 28 días previos para estimar el valor del día siguiente.
...
- **Ecuación o fundamento:** `X_t=[x_{t-28},\ldots,x_{t-1}],\quad y_t=y(t)`
...
- **Fuente:** 08_rnn_lstm_dataset_reducido.py
- **Hoja:** Sequential
- **Variables:** entrada, capa recurrente, Dense(16), Dense(1)
- **Objetivo:** Representar la arquitectura común utilizada para comparar RNN simple y LSTM.
...
- **Limitaciones:** No se realizó búsqueda exhaustiva de hiperparámetros; los resultados corresponden a una arquitectura base.
...
- **Interpretación:** La figura documenta una configuración base reproducible.
- **Criterio de lectura:** EarlyStopping puede detener el entrenamiento antes de las 80 épocas.
- **Ecuación o fundamento:** `\theta^*=\arg\min_\theta MAE_{val}(\theta)`
- **Uso sugerido en tesis:** Tabla o figura de configuración experimental.
- **Limitaciones:** El output actual no conserva el número real de épocas ejecutadas ni la historia de pérdida.
